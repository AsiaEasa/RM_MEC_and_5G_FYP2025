...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('22MAY-output.txt', mode='a', encoding='utf-8'),
...
class MECServer:
    def __init__(self, num_devices=NUM_DEVICES):
        ...
        self.optimizer = optim.Adam(self.global_model.parameters(), lr=0.001)  # أضف هذا
        self.total_distributed_samples = 0
    ...
    def distribute_data(self, trainset, selected_devices):
        ...
        for device_id in selected_devices:
            ...
            for label in range(10):
                ...
                self.total_distributed_samples += len(device_indices)
        ...
        if self.total_distributed_samples >= len(trainset):

            self.current_epoch += 1
            self.total_distributed_samples = 0
            logging.info(f"Epoch {self.current_epoch} completed.")
            return True
        return False
    ...
    def simulate_training_round(self, selected_devices, epochs=10, episode=0):
        ...
        epoch_completed = self.distribute_data(trainset, selected_devices)

        for device_id in selected_devices:
            device = self.devices[device_id]
            device.model.load_state_dict(self.global_model.state_dict())
            total_bandwidth += device.effective_bandwidth
            weights, T_local, T_trans, energy_used = device.train_local_model(epochs=5)
        ...
        return max_latency, total_energy, total_bandwidth, self.energy_history, epoch_completed
...
class DeviceSelectionEnv:
    ...
    def step(self, action, episode=0):
        ...
        max_latency, total_energy_consumed, total_bandwidth, e, epoch_completed = \
            self.mec_server.simulate_training_round(selected_indices, episode=episode)
        ...
        return state, reward, max_latency, epoch_completed
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_5g_lat22MAY.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    temp_latency = []
    temp_band = []
    MAX_EPOCHS = 100          # حد أقصى لعدد الإيبوك (للأمان)
    min_epochs_required = 5   # يجب أن يحقق الدقة المطلوبة لعدة إيبوك متتالية

    consecutive_success = 0   # عداد للإيبوك الناجحة
    iteration = 0

    # Create CSV file and write header
    with open("epoch_metrics.csv", "w", newline="", encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Epoch", "Precision", "Recall", "F1-score", "AUC"])
    ...
    for episode in range(NUM_EPISODES):
        ...
        while True:
            ...
            next_state, reward, max_latency, epoch_completed = env.step(action, episode)
            ...
            avg_reward = total_reward / episode_iterations if episode_iterations > 0 else reward
            accuracy, precision, recall, f1, auc1, all_preds, all_targets, all_probs = mec_server.evaluate_global_model(testloader)
             # حساب المقاييس الأساسية (macro averages مفيدة للـ multiclass)

             # ROC / AUC — تحويل الـ targets إلى one-hot (binarize) لمتعدد الفئات
            all_probs = np.array(all_probs)

            n_classes = all_probs.shape[1] if all_probs.ndim > 1 else 1
            y_true_bin = label_binarize(all_targets, classes=list(range(n_classes)))

            # roc_auc_score for multiclass: use 'ovr' or 'ovo' (one-vs-rest typical)
            #try:
            ...
            #episode_aucs.append(auc_macro if auc_macro is not None else 0)


            # Log/print
            logging.info(f"Accuracy: {accuracy:.4f}  Precision(macro): {precision:.4f}  Recall(macro): {recall:.4f}  F1(macro): {f1:.4f}")
           ...
           # if auc_macro is not None:
            ...
            temp_f1.append(f1)
            temp_auc.append(auc1)
            temp_energy.append(episode_energy)
            temp_latency.append(max_latency)
            temp_band.append(episode_bandwidth)
            ...
            logging.info(f"Iteration {iteration}, total_reward: {total_reward:.2f}, avg_reward: {avg_reward:.2f}, "
                        f"accuracy: {accuracy:.4f}, precision: {precision:.4f}, recall: {recall:.4f}, f1: {f1:.4f}, auc: {auc1:.4f}, "
                        f"max_accuracy: {max_accuracy:.2f}, energy: {episode_energy:.2f}, "
                        f"bandwidth: {episode_bandwidth:.2f}")
            if epoch_completed:
                print(f"\n========== Epoch {mec_server.current_epoch} Completed ==========")
                logging.info(f"========== Epoch {mec_server.current_epoch} Completed ==========")
                current_epoch_acc = np.min(temp_acc) if temp_acc else 0.0
                print(f"\n========== Epoch {mec_server.current_epoch} Completed - Min Acc = {current_epoch_acc:.4f} ==========")
                # استخدام متوسط المقاييس داخل الـ epoch

                epoch_accuracies.append(current_epoch_acc)
                epoch_precisions.append(np.mean(temp_prec))
                epoch_recalls.append(np.mean(temp_rec))
                epoch_f1s.append(np.mean(temp_f1))
                epoch_aucs.append(np.mean(temp_auc))
                epoch_energy.append(np.mean(temp_energy))
                epoch_latency.append(np.mean(temp_latency))
                epoch_bandwidth.append(np.mean(temp_band))

                logging.info(f"Epoch {mec_server.current_epoch} metrics saved.")
                # ====================== Early Stopping ======================

                WINDOW_SIZE = 5     # عدد epochs لمراقبة الاستقرار

                if len(epoch_accuracies) >= WINDOW_SIZE:

                    recent_acc = epoch_accuracies[-WINDOW_SIZE:]
                    avg_recent_acc = np.mean(recent_acc)

                    logging.info(
                        f"Recent Avg Accuracy ({WINDOW_SIZE} epochs): "
                        f"{avg_recent_acc:.4f}"
                    )

                    if avg_recent_acc >= DESIRED_ACCURACY:
                        logging.info(
                            f"🏁 Stop training "
                            f"(Average accuracy over last {WINDOW_SIZE} epochs "
                            f"= {avg_recent_acc:.4f})"
                        )
                        break

                # حد أمان فقط
                if mec_server.current_epoch >= MAX_EPOCHS:
        ...
        # Save metrics for this epoch to CSV
        with open("epoch_metrics.csv", "a", newline="", encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                mec_server.current_epoch,
                epoch_precisions[-1] if epoch_precisions else 0,
                epoch_recalls[-1] if epoch_recalls else 0,
                epoch_f1s[-1] if epoch_f1s else 0,
                epoch_aucs[-1] if epoch_aucs else 0
            ])
    classes = [str(i) for i in range(10)]
    plot_confusion_matrix(all_targets_list[-1], all_preds_list[-1], classes) #
    plt.savefig('confusion_matrix_fixed_5g_lat_EP22.png')  # حفظ مصفوفة الالتباس
    plt.close()
    
    # رسم منحنيات ROC بنفس الطريقة، باستخدام البيانات من التكرار 160
    plot_roc_curves(all_targets_list[-1], all_probs_list[-1], classes)
    plt.savefig('roc_curves_fixed_5g_lat_EP22.png')  # حفظ منحنيات ROC
    plt.close()
    ...
    x = range(1, len(epoch_accuracies)+1)

    plt.plot(x, epoch_accuracies,
            label='Accuracy',
            linewidth=2)

    plt.plot(x, epoch_precisions,
            label='Precision',
            linewidth=2)

    plt.plot(x, epoch_recalls,
            label='Recall',
            linewidth=2)

    plt.plot(x, epoch_f1s,
            label='F1-score',
            linewidth=2)

    plt.plot(x, epoch_aucs,
            label='AUC',
            linewidth=2)
    ...
    plt.plot(epoch_energy, label="Energy per epoch")
    plt.xlabel("Epoch")
    plt.ylabel("Energy (unit)")
    plt.title("Energy Consumption")
    plt.legend()
    plt.savefig('energy per epoch_fixed_5g_lat_ep22.png')
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.plot(epoch_latency, label="Latency per epoch")   
    plt.xlabel("Epoch ")
    plt.ylabel("Latency (s)")
    plt.title("Latency per Epoch")
    plt.legend()
    plt.savefig('Latency per epoch_fixed_5g_lat_22.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per epoch1_fixed_5g_lat22.png')
    plt.close()
    ...
    if len(epoch_accuracies) >= 1:
        ...
        # استخدام جميع الـ epochs
        num_epochs = len(epoch_precisions)

        # البيانات المطلوبة
        prec_data = epoch_precisions
        recall_data = epoch_recalls
        f1_data = epoch_f1s
        ...
        plt.title(
            'Precision, Recall and F1-Score Across All Epochs',
            fontsize=14,
            fontweight='bold',
            pad=20
        )
        ...
        plt.savefig(
            'precision_recall_f1_all_epochs22.png',
            dpi=300,
            bbox_inches='tight'
        )
        ...
        logging.info(
            f"Combined plot saved for all {num_epochs} epochs."
        )

    else:
        logging.warning("Not enough epochs to plot Precision/Recall/F1")
