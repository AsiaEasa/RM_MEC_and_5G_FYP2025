...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('12nov-output.txt', mode='a'),
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_5g_lat12nov.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    if len(all_preds_list) >= 160 and len(all_targets_list) >= 160:
        ...
        plt.savefig('confusion_matrix_fixed_5g_lat_12nov.png')  # حفظ مصفوفة الالتباس
        plt.close()
    else:
    ...
    plt.legend()
    plt.savefig('energy per episode_fixed_5g_lat_12nov.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Latency per episode_fixed_5g_lat_12nov.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per iteration1_fixed_5g_lat_12nov.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per for 160 iteration1_fixed_5g_lat_v12nov.png')
    plt.close()
