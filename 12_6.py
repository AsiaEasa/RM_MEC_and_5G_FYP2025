#  مابيطلعهنا قررنا نعمل الايتريشن مالانهايه ومابتقيف الحلقه لانو سي فار 10 بيصل الدقه في الستينات بس
import numpy as np
import torch
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('1ep_11_7_1.txt', mode='a'),
...
class CNN(nn.Module):
    def __init__(self, input_channels, output_dim):
        ...
        # طبقة تصفيح 1: 64 فلترًا
        self.conv1 = nn.Conv2d(input_channels, 64, kernel_size=3, padding=1)
        # طبقة تجميع لتقليل الأبعاد
        self.pool1 = nn.MaxPool2d(2, 2)  # من 16x16 إلى 8x8
        # طبقة تصفيح 2: 64 فلترًا
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        # طبقة تجميع ثانية
        self.pool2 = nn.MaxPool2d(2, 2)  # من 8x8 إلى 4x4
        # طبقة تصفيح 3: 64 فلترًا
        self.conv3 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        # طبقة متصلة بالكامل
        self.fc1 = nn.Linear(64 * 4 * 4, 64)  # بعد التجميع: 4x4x64 = 1024
        self.fc2 = nn.Linear(64, output_dim)   # الإخراج بحجم عدد الأجهزة (4)

    def forward(self, x):
        x = torch.relu(self.conv1(x))  # (batch_size, 64, 16, 16)
        x = self.pool1(x)              # (batch_size, 64, 8, 8)
        x = torch.relu(self.conv2(x))  # (batch_size, 64, 8, 8)
        x = self.pool2(x)              # (batch_size, 64, 4, 4)
        x = torch.relu(self.conv3(x))  # (batch_size, 64, 4, 4)
        x = x.view(x.size(0), -1)      # تسطيح إلى (batch_size, 64*4*4)
...
if __name__ == "__main__":
    ...
    for episode in range(NUM_EPISODES):
        ...
        episode_energy_history.append(episode_energy if episode_iterations > 0 else 0)
        episode_bandwidth_history.append(episode_bandwidth if episode_iterations > 0 else 0)
    ...
    plt.subplot(1, 4, 4)
    plt.plot(episode_accuracies, label="Accuracy per Itration")
    plt.xlabel("Iteration")
    plt.ylabel("Accuracy")
    plt.title("Model Accuracy Over iteration")
    plt.legend()
