...
if __name__ == "__main__":
    ...
    trainset = FakeData(size=1000, image_size=(1,28,28), num_classes=10, transform=transform)
    trainloader = DataLoader(trainset, batch_size=85, shuffle=True)
    
    testset = FakeData(size=200, image_size=(1,28,28), num_classes=10, transform=transform)
    testloader = DataLoader(testset, batch_size=85, shuffle=False)
    ...
    for episode in range(NUM_EPISODES):
        ...
        while accuracy < DESIRED_ACCURACY:
            ...
            num_selected = 5
            action = [0] * NUM_DEVICES
            selected_indices = random.sample(range(NUM_DEVICES), num_selected)
