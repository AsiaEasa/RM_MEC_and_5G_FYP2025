...
if __name__ == "__main__":
    ...
    with open("episode_metrics.csv", "w", newline="", encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Episode", "Precision", "Recall", "F1-score", "AUC"])
    ...
    for episode in range(NUM_EPISODES):
        ...
        while accuracy < DESIRED_ACCURACY:
            ...
            if epoch_completed:
                ...
                epoch_accuracies.append(np.mean(temp_acc))
        ...
        # Save metrics for this episode to CSV
        with open("episode_metrics.csv", "a", newline="", encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                episode + 1,
    ...
    logging.info(f"Average Metrics up to epoch 160: Accuracy={avg_accuracy:.4f}, Precision={avg_precision:.4f}, "
    ...
    if len(all_preds_list) >= 160 and len(all_targets_list) >= 160:
        ...
        plt.savefig('confusion_matrix_fixed_5g_lat_15dec.png')  # حفظ مصفوفة الالتباس
        plt.close()
        ...
        plt.savefig('roc_curves_fixed_5g_lat_15dec.png')  # حفظ منحنيات ROC
        plt.close()
    else:
    ...
    plt.legend()
    plt.savefig('Accuracy per epoch1_fixed_5g_lat_15dec.png')
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.plot(epoch_accuracies[:160], label="Accuracy per iteration (first 160)")
    plt.xlabel("epoch")
    plt.ylabel("Accuracy")
    plt.title("Model accuracy during first 160 epoch")
    plt.legend()
    plt.savefig('Accuracy per for 160 iteration1_fixed_5g_lat_v15dec.png')
    plt.close()
    ...
    plt.rcParams.update({
        ...
        # ====== Combined Line Plot for Precision, Recall, F1-Score (First 160 Iterations) ======
    if len(iteration_accuracies) >= 1:

        # عدد التكرارات حتى 160
        ...
        logging.warning("Not enough iterations to plot Precision/Recall/F1")
