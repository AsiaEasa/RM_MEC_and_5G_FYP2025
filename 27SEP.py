#   غيرنا طريقه توزيع البيانااتت+  غيرنا النقطتين المن تيليغرام 4 و 7  لحد اخر مييت
import numpy as np
import torch
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('miniset_fixed_data_set_sep27.txt', mode='a'),
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_MNISET27sep.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.savefig('energy per episode.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('Latency per episode.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('Accuracy per iteration.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('Accuracy per for 160 iteration.png')
    plt.show()
