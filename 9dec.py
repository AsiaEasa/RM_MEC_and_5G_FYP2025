...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('9dec-output.txt', mode='a', encoding='utf-8'),
...
class MECServer:
    ...
    def evaluate_global_model(self, testloader):
        ...
        return accuracy, precision, recall, f1, auc, all_preds, all_targets
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_5g_lat9dec.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    for episode in range(NUM_EPISODES):
        ...
        while accuracy < DESIRED_ACCURACY:
            ...
            accuracy, precision, recall, f1, auc, all_preds, all_targets = mec_server.evaluate_global_model(testloader)
    ...
    if len(all_preds_list) >= 160 and len(all_targets_list) >= 160:
        ...
        plt.savefig('confusion_matrix_fixed_5g_lat_4dec.png')
        plt.close()
    else:
    ...
    plt.legend()
    plt.savefig('energy per episode_fixed_5g_lat_4dec.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Latency per episode_fixed_5g_lat_4dec.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per iteration1_fixed_5g_lat_4dec.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per for 160 iteration1_fixed_5g_lat_v4dec.png')
    plt.close()
    ...
    plt.rcParams.update({
        ...
        # 7. Line plots for metrics over first 160 iterations (REPLACED: Separate line plots with full data for each metric)
    if len(iteration_accuracies) >= 1:  # نغير الشرط ليسمح بأقل من 160، لكن نحدد حتى 160
        # تحديد عدد التكرارات الفعلي (حتى 160)
        num_iters = min(160, len(iteration_accuracies))
        
        # استخراج البيانات حتى 160
        acc_data = iteration_accuracies[:num_iters]
        prec_data = iteration_precisions[:num_iters]
        recall_data = iteration_recalls[:num_iters]
        f1_data = iteration_f1s[:num_iters]
        auc_data = iteration_aucs[:num_iters]
        
        # قائمة لربط كل مقياس ببياناته، اسمه، ولونه
        metric_configs = [
            ('Accuracy', acc_data, sns.color_palette("husl", 5)[0]),  # أزرق
            ('Precision', prec_data, sns.color_palette("husl", 5)[1]),  # برتقالي
            ('Recall', recall_data, sns.color_palette("husl", 5)[2]),   # أخضر
            ('F1-Score', f1_data, sns.color_palette("husl", 5)[3]),     # أرجواني
            ('AUC', auc_data, sns.color_palette("husl", 5)[4])          # أحمر
        ]
        
        x = list(range(1, num_iters + 1))  # محور X: التكرارات من 1 إلى num_iters
        
        for metric_name, y_data, color in metric_configs:
            plt.figure(figsize=(10, 6))  # حجم مناسب للخط الكامل
            plt.plot(x, y_data, color=color, linewidth=2, marker='o', markersize=4, markevery=[-1])  # تعديل: إزالة label تمامًا لعدم ظهور في legend
            plt.xlabel('Iteration', fontsize=12)
            plt.ylabel('Score', fontsize=12)
            plt.title(f'{metric_name} during{num_iters} Iterations', fontsize=14, fontweight='bold', pad=20)
            plt.ylim(0, 1)
            plt.grid(axis='both', linestyle='--', alpha=0.7)
            
            
            # إضافة قيمة نهائية نصية عند النهاية (التكرار الأخير)
            final_value = y_data[-1]
            plt.text(x[-1], final_value + 0.01, f'{final_value:.3f}', ha='center', va='bottom', 
                    fontweight='bold', fontsize=11, color=color, bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
            
            plt.tight_layout()
            # حفظ كصورة مستقلة لكل مقياس (استخدام "_" للأسماء غير المدعومة)
            safe_name = metric_name.lower().replace('-', '_').replace(' ', '_')
            plt.savefig(f'{safe_name}_line_first_160_en.png', dpi=300, bbox_inches='tight')
            plt.close()
        
        # تسجيل القيم النهائية للتكرار 160 (أو الأخير)
        logging.info(f"Final Metrics for First {num_iters} Iterations: Accuracy={acc_data[-1]:.4f}, Precision={prec_data[-1]:.4f}, "
                    f"Recall={recall_data[-1]:.4f}, F1={f1_data[-1]:.4f}, AUC={auc_data[-1]:.4f}")
        logging.info(f"Separate line plots with full data saved for each metric over first {num_iters} iterations.")
    else:
        logging.warning("Not enough iterations to plot metrics line over first 160 (zero iterations)")


   # 7b. Line plots for metrics at iteration 160 (REPLACED: Separate line plots with single point for each metric)
    if len(iteration_accuracies) >= 160:
        ...
        # قائمة لربط كل مقياس بقيمته وألوانه
        metric_data = list(zip(metrics, values, colors))
       
        for i, (metric_name, value, color) in enumerate(metric_data):
            plt.figure(figsize=(8, 6))  # حجم أصغر للرسوم المنفصلة
            x = [160]  # محور X: التكرار 160 كقيمة واحدة للخط
            y = [value]
            plt.plot(x, y, marker='o', color=color, linewidth=2, markersize=8, label=metric_name)  # خط مع نقطة
            plt.xlabel('Iteration', fontsize=12)
            plt.ylabel('Score', fontsize=12)
            plt.title(f'{metric_name} - Iteration 160', fontsize=14, fontweight='bold', pad=20)
            plt.ylim(0, 1)
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.legend()
           
            # إضافة قيمة نصية على النقطة
            plt.text(x[0], value + 0.01, f'{value:.3f}', ha='center', va='bottom', 
                     fontweight='bold', fontsize=11, color=color)
           
            plt.tight_layout()
            # حفظ كصورة مستقلة لكل مقياس (مع "_single" للتمييز)
            safe_name = metric_name.lower().replace("-", "_").replace(" ", "_")
            plt.savefig(f'{safe_name}_line_single_160_en.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        logging.info("Separate single-point line plots saved for each metric at iteration 160.")
    else:
        logging.warning("Not enough iterations to plot single-point metrics line for iteration 160 (less than 160)")
