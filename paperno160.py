...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('p-output.txt', mode='a', encoding='utf-8'),
...
class DDQNAgent:
    ...
    def preprocess_state(self, state):
        ...
        # تطبيع
        state_array = (state_array - np.mean(state_array)) / (np.std(state_array) + 1e-8)

        target_size = 256

        # إذا كان أكبر من 256 نقصه
        if len(state_array) > target_size:
            state_array = state_array[:target_size]

        # إذا كان أصغر من 256 نضيف padding
        elif len(state_array) < target_size:
            pad_size = target_size - len(state_array)
            state_array = np.pad(state_array, (0, pad_size), mode='constant')

        return state_array.reshape(1, 16, 16)
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_5g_latp.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=85, shuffle=True, num_workers=0)
    testset = torchvision.datasets.MNIST(root='./data', train=False, download=True, transform=transform)
    testloader = torch.utils.data.DataLoader(testset, batch_size=85, shuffle=False, num_workers=0)
    ...
    num_iters_for_avg = len(iteration_accuracies)
    ...
    logging.info(f"Average Metrics up to Iteration {num_iters_for_avg}: Accuracy={avg_accuracy:.4f}, Precision={avg_precision:.4f}, "
                 f"Recall={avg_recall:.4f}, F1={avg_f1:.4f}, AUC={avg_auc:.4f}")
    # رسم مصفوفة الالتباس باستخدام التنبؤات من التكرار الأخير
    if len(all_preds_list) > 0 and len(all_targets_list) > 0:
        ...
        plt.savefig('confusion_matrix_fixed_5g_lat_p.png')  # حفظ مصفوفة الالتباس
        plt.close()

        # رسم منحنيات ROC بنفس الطريقة، باستخدام البيانات من التكرار الأخير
        plot_roc_curves(all_targets_list[-1], all_probs_list[-1], classes)
        plt.savefig('roc_curves_fixed_5g_lat_15dec.png')  # حفظ منحنيات ROC
        plt.close()
    else:
        logging.warning("Cannot plot confusion matrix/ROC curves: no iterations completed or no data")
    ...
    plt.legend()
    plt.savefig('energy per episode_fixed_5g_lat_p.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Latency per episode_fixed_5g_lat_p.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per iteration1_fixed_5g_lat_p.png')
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.plot(iteration_accuracies, label="Accuracy per iteration (all iterations)")
    plt.xlabel("Iteration")
    plt.ylabel("Accuracy")
    plt.title("Model accuracy during all iterations")
    plt.legend()
    plt.savefig('Accuracy per for all iteration1_fixed_5g_lat_p.png')
    plt.close()
    ...
    plt.rcParams.update({
        ...
        # ====== Combined Line Plot for Precision, Recall, F1-Score (All Iterations) ======
    if len(iteration_accuracies) >= 1:

        # عدد التكرارات حتى 160
        num_iters = len(iteration_precisions)
        ...
        # حفظ الشكل
        plt.tight_layout()
        plt.savefig('precision_recall_f1_all_iterations15dec.png',
        ...
        logging.info("Metrics averages plotted successfully!")
