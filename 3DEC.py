...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('3DEC-output.txt', mode='a'),
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_5g_lat3DEC.txt", "a") as f:
...
#************* 
def compute_advanced_metrics(model, testloader, device="cpu"):
    model.eval()
    all_preds = []
    all_targets = []
    all_probs = []
    ...
    try:
        ...
        auc_macro = roc_auc_score(y_bin, all_probs, average='macro')
        auc_weighted = roc_auc_score(y_bin, all_probs, average='weighted')
    except:
        auc_macro = auc_weighted = 0.5
...
if __name__ == "__main__":
 ...
 #*************
    for episode in range(NUM_EPISODES):
        ...
        while accuracy < DESIRED_ACCURACY:
            ...
            logging.info(f"Iteration {iteration}, total_reward: {total_reward:.2f}, avg_reward: {avg_reward:.2f}, "
                        ...
                        # === حساب المقاييس المتقدمة كل 5 تكرارات فقط (لتوفير الوقت) ===

             #*************
            if iteration % 160 == 0 or accuracy >= DESIRED_ACCURACY:
                metrics = compute_advanced_metrics(mec_server.global_model, testloader)
                precision_history.append(metrics['precision'])
                recall_history.append(metrics['recall'])
                f1_history.append(metrics['f1'])
                auc_macro_history.append(metrics['auc_macro'])
                auc_weighted_history.append(metrics['auc_weighted'])
                logging.info(f"Iter {iteration} | Prec: {metrics['precision']:.4f} | Rec: {metrics['recall']:.4f} | "
                             f"F1: {metrics['f1']:.4f} | AUC: {metrics['auc_macro']:.4f}")
            else:
                # نضيف قيم وهمية للحفاظ على طول القوائم (للرسم)
                precision_history.append(precision_history[-1] if precision_history else 0.1)
                recall_history.append(recall_history[-1] if recall_history else 0.1)
                f1_history.append(f1_history[-1] if f1_history else 0.1)
                auc_macro_history.append(auc_macro_history[-1] if auc_macro_history else 0.5)
                auc_weighted_history.append(auc_weighted_history[-1] if auc_weighted_history else 0.5)   
    ...
    if len(all_preds_list) >= 160 and len(all_targets_list) >= 160:
        ...
        plt.savefig('confusion_matrix_fixed_5g_lat_3DEC.png')  # حفظ مصفوفة الالتباس
        plt.close()
    else:
    ...
    plt.savefig('energy per episode_fixed_5g_lat_3DEC.png')
    plt.close()
    ...
    plt.savefig('Latency per episode_fixed_5g_lat_3DEC.png')
    plt.close()
    ...
    plt.savefig('Accuracy per iteration1_fixed_5g_lat_3DEC.png')
    plt.close()
 #*************
    its = range(0, 160, 5)  # للمقاييس المتقدمة
    plt.figure(figsize=(10, 6))
    ...
    plt.legend()
    plt.savefig('Accuracy per for 160 iteration1_fixed_5g_lat_3DEC.png')
    plt.close()
    plt.figure(figsize=(12, 8))
    plt.plot(its, precision_history[:len(its)], label='Precision (macro)', color='blue', linewidth=2)
    plt.plot(its, recall_history[:len(its)], label='Recall (macro)', color='green', linewidth=2)
    plt.plot(its, f1_history[:len(its)], label='F1-Score (macro)', color='red', linewidth=2.5)
    plt.plot(its, auc_macro_history[:len(its)], label='AUC (macro)', color='purple', linewidth=2)
    plt.plot(its, auc_weighted_history[:len(its)], label='AUC (weighted)', color='orange', linestyle='--')
    plt.axhline(y=0.86, color='gray', linestyle=':', label='Target 0.86')
    plt.xlabel('Iteration')
    plt.ylabel('Score')
    plt.title('Advanced Metrics During First 160 Iterations')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('advanced_metrics_160_iterations_oct24.png')
    plt.close()
#*************
