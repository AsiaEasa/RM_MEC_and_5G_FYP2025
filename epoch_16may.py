...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('15dec-output.txt', mode='a', encoding='utf-8'),
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_5g_lat15dec.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    plt.savefig('confusion_matrix_fixed_5g_lat_EP.png')  # حفظ مصفوفة الالتباس
    plt.close()
    
    # رسم منحنيات ROC بنفس الطريقة، باستخدام البيانات من التكرار 160
    plot_roc_curves(all_targets_list[-1], all_probs_list[-1], classes)
    plt.savefig('roc_curves_fixed_5g_lat_EP.png')  # حفظ منحنيات ROC
    plt.close()
    ...
    plt.legend()
    plt.savefig('energy per epoch_fixed_5g_lat_ep.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Latency per epoch_fixed_5g_lat_.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per epoch1_fixed_5g_lat.png')
    plt.close()
    ...
    if len(epoch_accuracies) >= 1:
        ...
        plt.savefig(
            'precision_recall_f1_all_epochs.png',
            dpi=300,
            bbox_inches='tight'
        )
