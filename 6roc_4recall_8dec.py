...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('8dec-output.txt', mode='a', encoding='utf-8'),
...
class MECServer:
    ...
    def evaluate_global_model(self, testloader):
        ...
        return accuracy, precision, recall, f1, auc, all_preds, all_targets, all_probs_np
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_5g_lat8dec.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    if len(all_preds_list) >= 160 and len(all_targets_list) >= 160:
        ...
        plt.savefig('confusion_matrix_fixed_5g_lat_8dec.png')
        plt.close()
        ...
        plt.savefig('roc_curves_fixed_5g_lat_8dec.png')  # حفظ منحنيات ROC
    ...
    plt.legend()
    plt.savefig('energy per episode_fixed_5g_lat_8dec.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Latency per episode_fixed_5g_lat_8dec.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per iteration1_fixed_5g_lat_8dec.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per for 160 iteration1_fixed_5g_lat_v8dec.png')
    plt.close()
    ...
    if len(iteration_accuracies) >= 160:
        ...
        plt.title('Performance Metrics - Iteration 160', fontsize=14, fontweight='bold', pad=20)
        plt.ylim(0, 1)
