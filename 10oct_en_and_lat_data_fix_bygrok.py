#  |غيرنا عشان نحسن الطاقه والتاخيروالبيانات
import numpy as np
import torch
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fixed_en_lat_oct10+DATA.txt', mode='a'),
...
class MECServer:
    ...
    def initialize_class_indices(self, trainset):
        ...
        for label in self.class_indices:
            random.shuffle(self.class_indices[label])  # خلط لتجنب أنماط ثابتة
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_en_lat_oct10.txt+DATA", "a") as f:
...
if __name__ == "__main__":
    ...
    if len(all_preds_list) >= 160 and len(all_targets_list) >= 160:
        ...
        plt.savefig('confusion_matrix_fixed_en_lat_oct10+DATA.png')  # حفظ مصفوفة الالتباس
        plt.close()
    else:
    ...
    plt.legend()
    plt.savefig('energy per episode_fixed_en_lat_oct10 +DATA.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Latency per episode_fixed_en_lat_oct10+DATA.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per iteration1_fixed_en_lat_oct10+DATA.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('Accuracy per for 160 iteration1 _fixed_en_lat_oct10+DATA.png')
    plt.close()
